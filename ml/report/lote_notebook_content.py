...
ACRONYMS: list[tuple[str, str]] = [
    ...
    ("PASTIS-R", "Panoptic Agricultural Satellite TIme Series (version Radar). Dataset "
    ...
    ("NDVI", "Normalized Difference Vegetation Index: indice de vigor de la vegetacion "
    ...
    ("CLS", "Token de Clasificacion (Class token): el vector resumen de toda la imagen "
     "que produce un Vision Transformer."),
    ("mIoU", "mean Intersection over Union: promedio del solapamiento entre prediccion "
    ...
    ("F1-macro", "Promedio simple del F1 (media armonica de precision y exhaustividad) "
     "sobre todas las clases; trata igual a clases raras y comunes."),
    ("OOF", "Out-Of-Fold: predicciones hechas sobre datos que el modelo NO vio al "
    ...
    ("MPCL", "Multi-Positive Contrastive Loss: perdida contrastiva que agrupa varios "
     "ejemplos positivos de la misma categoria (nucleo del FarSLIP fiel)."),
    ("L_glo / L_loc", "Perdidas global (imagen<->caption) y local (region<->categoria) "
     "del paper FarSLIP."),
    ...
    ("Full-M", "Configuracion 'Medium' completa del TSViT (dim 192, 6+6 capas), la del "
...
GLOSSARY: list[tuple[str, str]] = [
    ...
    ("Patch",
     "Recorte cuadrado de 128x128 pixeles del satelite. La unidad basica del dataset."),
    ("Embedding",
