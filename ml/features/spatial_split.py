...
def build_spatial_kfold(
    ...
    *,
    ...
    if parcels.geometry.name not in parcels.columns:
        raise ValueError("`parcels` debe tener geometría activa (parcels.set_geometry('geom')).")
...
def _assign_h3_cell(centroid: Point, h3_res: int) -> str:
    ...
    if hasattr(h3, "latlng_to_cell"):  # h3-py 4.x
        ...
        raise ImportError("h3-py expone una API desconocida; ni `latlng_to_cell` ni `geo_to_h3`.")
