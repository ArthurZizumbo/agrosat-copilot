...
__all__ = ["build_objective", "run_study"]

#: Modelos temporales soportados (el warm-start asume encoder no congelado).
_SUPPORTED = ("tsvit", "tsvit-pheno")
    ...
    *,
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        train_loader = DataLoader(
            ...
            drop_last=False, pin_memory=amp_enabled,
        )
        ...
        for epoch in range(epochs):
            _run_epoch(
                model, train_loader, criterion=criterion, device=dev,
                use_phenology=use_phenology, prototypes=prototypes,
                lambda_contrast=lambda_contrast, ignore_index=255,
                optimizer=optimizer, scaler=scaler, use_amp=amp_enabled,
            )
            metrics, _ = _evaluate_dense(
                model, val_loader, device=dev, num_classes=num_classes,
                ignore_index=255, use_phenology=use_phenology,
            )
...
def run_study(
    ...
    *,
    ...
    Args:
        ...
        init_ckpt: ``best.pt`` del modelo base (warm-start).
    ...
    objective, base_metrics = build_objective(
        model_kind, init_ckpt=init_ckpt, epochs=epochs, n_timesteps=n_timesteps,
        target=target, device=device, train_folds=train_folds, val_folds=val_folds,
        num_workers=num_workers, lambda_contrast=lambda_contrast,
    )
    ...
    logger.info(
        "study_done",
        model=model_kind, n_trials=len(study.trials), best_value=study.best_value,
        base_miou=base_miou, elapsed_s=round(elapsed, 1),
    )
