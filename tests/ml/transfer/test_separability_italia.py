...
def test_class_separability_gates() -> None:
    ...
    starved = ClassSeparability(
        class_id=1, name="sunflower", n_parcels=_MIN_PARCELS_FOR_TARGET - 1,
        mean_jm=2.0, nearest_name="olive", nearest_jm=2.0,
    )
    ...
    overlapping = ClassSeparability(
        class_id=2, name="barley", n_parcels=_MIN_PARCELS_FOR_TARGET + 10,
        mean_jm=0.5, nearest_name="oats", nearest_jm=_JM_SEPARABLE - 0.1,
    )
    ...
    rescuable = ClassSeparability(
        class_id=3, name="olive", n_parcels=_MIN_PARCELS_FOR_TARGET + 100,
        mean_jm=1.8, nearest_name="vineyards", nearest_jm=_JM_SEPARABLE + 0.5,
    )
