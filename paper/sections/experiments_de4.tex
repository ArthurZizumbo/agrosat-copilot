...
\paragraph{How the Tuscany ceiling was reached: three transfer strategies.}
Before DE4, the same France-trained backbone (TSViT-pheno) was carried to the Italian
Tuscany dataset ($39$ native classes, test fold-3) under three transfer strategies, to
separate what the transfer actually buys. \emph{(i)} Training from scratch with Italian
data only reaches macro F1~$0.095$. \emph{(ii)} Pure transfer --- reusing France's temporal
encoder and learning a fresh classifier head for the Italian crops --- lifts it to $0.120$.
\emph{(iii)} Keeping the French classes (warm-starting the head from the French crops) or
replicating the full PASTIS training mode reaches $0.130$--$0.132$. The temporal encoder is
therefore the load-bearing transferred component; warm-starting the head adds little on
average ($+0.012$) and is \emph{not} universal --- it helps crops that look alike across
France and Italy (durum wheat $+0.08$, barley $+0.07$, alfalfa $+0.05$) and hurts those with
a different Mediterranean phenology (oats $-0.08$, sunflower $-0.06$). This $\approx0.13$
best-case Tuscany ceiling is the baseline that the DE4 levers below lift to $0.266$.
% src: reports/us079_figs/ablation_compare.json (A_warmstart 0.132, B_nowarmstart 0.120,
% original 0.130; from-scratch 0.095; per-class warm-start deltas)
...
\paragraph{Three label-space conventions.}
