...
demostración cualitativa zero-shot sobre México y una extensión tropical
explícitamente fuera de alcance. La tesis que defendemos es
...
Gemini~2.5~Pro~\cite{google2025gemini25} (GA, contexto de 1M tokens);
...
áreas de demostración cualitativa de México.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\linewidth]{figures/us-070/aoi_italy.png}
  \caption{Área de interés italiana usada para la destilación contrastiva
  de FarSLIP (\texttt{farslip-clip-italy-v1}), un nodo de la huella
  multirregión de AgroSatCopilot. Mapa base y superposición del AOI;
  contiene datos Sentinel de Copernicus modificados. AlphaEarth
...
LV$+$PT~$\rightarrow$~EE y LV~$\rightarrow$~EE (Reuter et al.
...
\subsection{Demostración cualitativa zero-shot: México}
\label{sec:multiregion:mexico}

Para mostrar que el mismo pipeline se replica fuera de Europa, lo
aplicamos a dos áreas productoras mexicanas reales \emph{sin} verdad de
terreno curada: aguacate en torno a Uruapan/Tancítaro (Michoacán) y
guayaba en torno a Calvillo (Aguascalientes). Para cada área de interés
obtenemos el embedding zonal real de AlphaEarth de 64 dimensiones y la
serie temporal real de NDVI Sentinel-2 desde Google Earth Engine, y
alineamos la narrativa fenológica a la familia
\texttt{PERMANENT\_WOODY} del espacio macro HCAT como una analogía
cualitativa. La Figura~\ref{fig:mexico_phenology} muestra las dos curvas
NDVI: ambos cultivos son leñosos perennes (similares a un huerto), con
una señal relativamente alta y estable en lugar del ciclo
siembra--cosecha de un cereal anual, y el AOI de guayaba muestra la
modulación esperada de fin de temporada por el riego y la poda. Esta
demostración es puramente metodológica: como no hay verdad de terreno,
\textbf{nunca reportamos un F1, exactitud ni clasificador sobre México}
--- un metatest en el código fuente impone que ninguna métrica de
clasificación se importe en esta ruta. Ilustra el mecanismo de ``recibir
nuevas clases'', no una afirmación de desempeño. AlphaEarth (Satellite
Embedding V1 Annual, CC-BY-4.0);
contiene datos Sentinel de Copernicus modificados.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/us-073-transfer/mexico_phenology.png}
  \caption{Demostración cualitativa de México: fenología NDVI Sentinel-2
  real de aguacate (Uruapan, Michoacán) y guayaba (Calvillo,
  Aguascalientes), ambos cultivos leñosos perennes. No existe verdad de
  terreno para estos AOI, de modo que no se calcula ni se implica ningún
  F1/exactitud. La figura ilustra la metodología de extender a una nueva
  región, no una métrica.}
...
modelo sigue siendo útil como un vocabulario \emph{macro} más rico y un
generador de hipótesis de grano fino (la demostración cualitativa de
México mapea con confianza, y erróneamente, los huertos tropicales a
\texttt{grassland}, sin una clase de huerto tropical que ofrecer), pero
...
de datos on-premises vía Qwen3.5-35B-A3B~\cite{yang2025qwen3} servido con
vLLM GPTQ-Int4 en una sola GPU), ni ningún F1 $\geq$0.80 en México o los
