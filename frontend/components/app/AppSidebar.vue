...
const aois = computed(() => (activeAoi.value ? [activeAoi.value] : []));
      ...
      <section v-if="!rail">
        ...
        <ul v-else class="space-y-1">
          <li v-for="aoi in aois" :key="aoi.id">
            <div
              class="flex w-full items-center justify-between gap-2 rounded-[var(--radius-sm)] bg-agro-50 px-2.5 py-2 text-left text-sm text-agro-800 dark:bg-agro-900/40 dark:text-agro-300"
            >
