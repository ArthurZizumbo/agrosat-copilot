...
def _load_parcels(path: Path) -> gpd.GeoDataFrame:
    ...
    if path.suffix in (".geoparquet", ".parquet"):
        ...
        raise typer.BadParameter(f"Formato no soportado: {path.suffix}. Usa .parquet o .geojson.")
    if "parcel_id" not in gdf.columns:
        raise typer.BadParameter(f"Parcels en {path} no contiene la columna `parcel_id`.")
    return gdf
