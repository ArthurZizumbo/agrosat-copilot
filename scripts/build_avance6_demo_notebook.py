...
def _build_cells() -> list:
    ...
    cells.append(
        md(
            ...
            "rastreable.\n\n"
            "Seis son **sincronas** (se ejecutan en linea dentro del bucle del agente: listar "
            "parcelas, serie temporal, estadisticas de un area, clasificar, explicar y **recuperar "
            "contexto del RAG** para anclar la respuesta). Las otras cuatro son **diferidas** "
            "(*deferred*): se completan fuera de linea via un *worker* (buscar escenas, teselas de "
            "mapa, guardar un area, comparar modelos). El Spatial-RAG entra al bucle **solo cuando "
            "`rag_enabled`** esta activo, para que el reasoner pueda anclarse en evidencia citada "
            "sin alucinar. La tabla se construye desde `build_function_declarations()` -- la misma "
            "fuente de verdad que se le anuncia al LLM."
        )
    )
    cells.append(
        code(
    ...
    cells.append(
        md(
            ...
            "- Un **conjunto cerrado de diez herramientas** con esquemas validados: las seis "
            "sincronas (incluido el **Spatial-RAG** que ancla la respuesta cuando `rag_enabled`) "
            "y, en crudo, las de area y diferidas (`compare_models`, `add_aoi`, ...).\n"
