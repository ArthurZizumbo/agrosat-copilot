...
def _patch_ids(filtered_splits_path: Path | None = None,
               splits: list[str] | None = None) -> list[str]:
    """Return sorted PASTIS-R patch ids, optionally filtered by the parquet
    generated by pastis_filter_farslip.ipynb.

    Args:
        filtered_splits_path: path to pastis_farslip_filtered_splits.parquet.
                              If None, uses all patches with S2+target+instance.
        splits: which splits to include, e.g. ['train', 'val', 'test'].
                None = all splits in the parquet.
    """
    if filtered_splits_path is not None and Path(filtered_splits_path).is_file():
        import polars as pl
        df = pl.read_parquet(filtered_splits_path)
        if splits:
            df = df.filter(pl.col('split').is_in(splits))
        # Keep only patches that also have instance annotations on disk
        all_ids = [str(pid) for pid in df['patch_id'].to_list()]
        valid = []
        for pid in all_ids:
            if (
                (_PASTIS / 'DATA_S2' / f'S2_{pid}.npy').is_file()
                and (_PASTIS / 'ANNOTATIONS' / f'TARGET_{pid}.npy').is_file()
                and (_PASTIS / 'INSTANCE_ANNOTATIONS' / f'INSTANCES_{pid}.npy').is_file()
            ):
                valid.append(pid)
        _log.info('filtered_patch_ids', total_in_parquet=len(all_ids),
                  valid_on_disk=len(valid), splits=splits)
        return sorted(valid)

    # Default: all patches with all required files
    s2_dir = _PASTIS / 'DATA_S2'
    ids = []
    for f in sorted(s2_dir.glob('S2_*.npy')):
        pid = f.stem.replace('S2_', '')
        if (
            (_PASTIS / 'ANNOTATIONS' / f'TARGET_{pid}.npy').is_file()
            and (_PASTIS / 'INSTANCE_ANNOTATIONS' / f'INSTANCES_{pid}.npy').is_file()
        ):
            ids.append(pid)
    return ids
...
def _iter_parcel_crops(pid: str, min_px: int,
                       classes_of_interest: set[int] | None = None):
    """Yield (parcel_id, class_id, rgb_pil) for each valid parcel in a patch.
    If classes_of_interest is given, only parcels whose majority class is in
    that set are yielded.
    """
    s2 = np.load(_PASTIS / "DATA_S2" / f"S2_{pid}.npy").astype(np.float32)
    median = np.median(s2, axis=0)
    ...
    for iid in np.unique(inst):
        ...
        if classes_of_interest and class_id not in classes_of_interest:
            continue
...
def main(
    ...
    filtered_splits: Annotated[
        Path | None,
        typer.Option(help="Parquet generado por pastis_filter_farslip.ipynb. "
                          "Si se omite, se procesan todos los patches.")
    ] = None,
    splits: Annotated[
        str | None,
        typer.Option(help="Splits a incluir, separados por coma: train,val,test. "
                          "Solo aplica si se pasa --filtered-splits.")
    ] = None,
    classes_of_interest: Annotated[
        str | None,
        typer.Option(help="Clases a incluir (indices separados por coma: 2,3,8). "
                        "None = todas las clases presentes en los patches.")
    ] = None,
) -> None:
    ...
    splits_list = [s.strip() for s in splits.split(',')] if splits else None
    pids = _patch_ids(
        filtered_splits_path=filtered_splits,
        splits=splits_list,
    )
    if max_patches > 0:
    ...
    coi_set = {int(c) for c in classes_of_interest.split(',')} if classes_of_interest else None

    for i, pid in enumerate(pids):
        for parcel_id, class_id, rgb in _iter_parcel_crops(pid, min_parcel_px,
                                                            classes_of_interest=coi_set):
